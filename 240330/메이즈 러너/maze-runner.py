...
for _ in range(M):
    i, j = map(lambda x:int(x)-1, input().split()) # 1빼서 입력 받음
    arr[i][j] -= 1 # 사람은 (같은 위치에 여러명 있을 수 있다)
...
for _ in range(K):
    ...
    # 출구에 도착하면 즉시 탈출
    narr = [x[:] for x in arr] # 복사 -> deepcopy보다 빠름
    for i in range(N):
        for j in range(N):
            if -11<arr[i][j]<0: # 사람인 경우
                dist = abs(ei-i) + abs(ej-j)
                # 네 방향(상하우선), 범위 내, 벽 아니고 : <=0, 거리가 dist보다 작으면
                for di, dj in ((-1,0),(1,0),(0,-1),(0,1)):
                    ni, nj = i + di, j + dj
                    if 0<=ni<N and 0<=nj<N and arr[ni][nj] <= 0 and dist > (abs(ei-ni)+abs(ej-nj)):
                        ans += arr[i][j] # 현재 인원 수가 이동하는 것이니 이동거리에 누적
