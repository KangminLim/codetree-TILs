...
def leave(num,ci,cj,cd,cp,cg,cs):
    # 현재 방향부터 시계방향 90도씩 빈칸 찾기 ( 최소한 내가 온(상대방이 온)칸은 비어있음)
    for k in range(4):
...
for _ in range(K): # K라운드 동안 경기 진행
    ...
    for i in players:
        # [1] 한칸 이동(격자 벗어나면 반대 방향)
        ci,cj,cd,cp,cg,cs = players[i]
        ni,nj=ci+di[cd], cj+dj[cd]
        if ni < 0 or N<=ni or ni<0 or N<=nj: # 범위 벗어남
            cd = opp[cd]
            ni, nj = ci+di[cd], cj+dj[cd] # 한칸 이동
        arr[ci][cj] = 0 # 이전 위치에서 제거

        # [2-1] 이동한 위치가 빈칸인 경우 => 강한 총 획득
        if arr[ni][nj] == 0:
            if len(gun[ni][nj]) > 0: # 총이 있다면
                mx = max(gun[ni][nj])
                if cg < mx: # 더 강한 총이면 교체
                    if cg > 0: # 총이 있는 경우
                        gun[ni][nj].append(cg) #내 총을 반납
                    gun[ni][nj].remove(mx)
                    cg = mx
        ...
        # [2-2] 빈칸이 아닌 경우 => 싸울 상대방이 있는 경우
            ...
            if cp + cg > ep + eg or (cp+cg==ep+eg and cp > ep): # 내가 이기는 경우
                ...
                es += (ep + eg) - (cp + cg)  # 공격력 차이 만큼 점수 획득
                # 상대방은 총을 놓고 떠남
